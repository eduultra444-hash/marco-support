...
client.once("ready", async () => { console.log("Bot conectado como " + client.user.tag); try { const rest = new REST({ version: "10" }).setToken(process.env.BOT_TOKEN); await rest.put(Routes.applicationGuildCommands(process.env.CLIENT_ID, process.env.GUILD_ID), { body: commands }); console.log("/painelsuporte registrado."); } catch (error) { console.error("Erro ao registrar comando:", error); } });client.on("guildMemberAdd", async member => { const channel=member.guild.channels.cache.get("1550238149630754916"); if (!channel?.isTextBased()) return; const embed=new EmbedBuilder().setColor(0x57f287).setTitle("👋 Novo membro").setDescription("Bem-vindo(a), "+member+"!").addFields({name:"Usuário",value:member.user.username,inline:true},{name:"Perfil",value:"https://discord.com/users/"+member.id,inline:true}).setThumbnail(member.user.displayAvatarURL({size:512})).setFooter({text:"Marco Store"}); await channel.send({embeds:[embed]}).catch(console.error); });client.on("guildMemberRemove", async member => { const channel=member.guild.channels.cache.get("1550242627062861876"); if (!channel?.isTextBased()) return; const embed=new EmbedBuilder().setColor(0xed4245).setTitle("👋 Membro saiu").setDescription(member.user.username+" saiu do servidor.").addFields({name:"Usuário",value:member.user.username,inline:true},{name:"Perfil",value:"https://discord.com/users/"+member.id,inline:true}).setThumbnail(member.user.displayAvatarURL({size:512})).setFooter({text:"Marco Store"}); await channel.send({embeds:[embed]}).catch(console.error); });
